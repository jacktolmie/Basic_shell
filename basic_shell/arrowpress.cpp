...
size_t Basicshell::arrow_press(std::string& sent_command)
{
	while (true) {
		 ...
		 */
		if (keypress == 126) {
            Basicshell::delete_char(sent_command);
			return 0;
		}
		...
		switch (keypress) {
			...
			case 65: {
				...
				if (!previous_commands.empty() &&
                    ...
                    Basicshell::print_line(prev_cmd_holder);
				}
				continue;
			}
			...
			case 67: {
				if (cursor_moves > 0) {
					--cursor_moves;
                    Basicshell::print_line((!prev_cmd_holder.empty())
											  ? prev_cmd_holder
											  : sent_command);
				}
