...
size_t Basic_Shell::arrow_press(std::string& sent_command)
{
	while (true) {
		 ...
		 */
		if (keypress == 126) {
            Basic_Shell::delete_char(sent_command);
			return 0;
		}
		...
		switch (keypress) {
			...
			case 65: {
				...
				if (!previous_commands.empty() &&
                    ...
                    Basic_Shell::print_line(prev_cmd_holder);
				}
				continue;
			}
			...
			case 67: {
				if (cursor_moves > 0) {
					--cursor_moves;
                    Basic_Shell::print_line((!prev_cmd_holder.empty())
											  ? prev_cmd_holder
											  : sent_command);
				}
