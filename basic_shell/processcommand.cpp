...
void Basicshell::process_command(std::string& sent_command)
{
	...
	if (it.func_name != "noname") {
        ...
        arguments options{Basicshell::process_options(sent_command)};
		...
		if (it.func_name == "help" || it.func_name == "history") {
            ...
            Basicshell::add_command(sent_command);
			return;
		}
...
void Basicshell::add_command(std::string& sent_command)
{
